...
class FilesResource:
    ...
    async def on_post(self, request, response):
        ...
        try:
            ...
            if self._insert_file(tmp_post_body):
                # File inserted
            ...
            self._insert_mfa(tmp_media_result, tmp_file_result)
    ...
    def _insert_file(self, tmp_post_body):
        try:
    ...
    def _insert_mfa(self, tmp_media, tmp_file):
        try:
            ...
            with mmangler.models.db_session() as tmp_session:
                ...
                tmp_session.commit()
            self.logger.debug("Updated file medias: %s:%s", tmp_new_file.id, tmp_new_file.name)
        except Exception as ex:
            # Something else is wrong, so bail out
            self.logger.debug("Exception: %s", ex)
            raise ServerErrorException()
